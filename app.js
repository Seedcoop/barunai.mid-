...
async function buildProgressiveReply(question) {
  ...
  const ethicsPlayReply = buildEthicsPlayReplyIfNeeded(question);
  if (ethicsPlayReply) {
    return {
      text: ethicsPlayReply,
      source: "local"
    };
  }
...
function buildEthicsPlayReplyIfNeeded(question) {
  const active = new Set(state.activeGuideFeatures);
  if (asksCopyrightQuestion(question)) {
    return active.has("respect") ? buildSafeCopyrightReply() : buildUnsafeCopyrightPlayReply(question);
  }
  if (asksResponsibilityQuestion(question)) {
    return active.has("responsibility") ? buildSafeResponsibilityReply() : buildUnsafeResponsibilityPlayReply();
  }
  if (asksAgencyQuestion(question)) {
    return active.has("agency") ? buildSafeAgencyReply() : buildUnsafeAgencyPlayReply();
  }
  return "";
}
...
function analyzeAssistantAnswer(text) {
  ...
  const conductRisk = calculateConductRisk(lower);
  ...
  const ethicsRisk = calculateEthicsRisk({ biasRisk, hallucinationRisk, groundingScore, safetyRisk, conductRisk });
  const comment = buildEthicsRiskComment({
    ethicsRisk,
    biasRisk,
    hallucinationRisk,
    groundingScore,
  ...
  return { biasRisk, hallucinationRisk, groundingScore, safetyRisk, conductRisk, ethicsRisk, comment };
}
...
function calculateConductRisk(lower) {
  const copyrightRiskTerms = [
    "출처는 시간이 없으면 생략",
    "출처는 생략",
    "가져와 써도",
    "필요한 만큼 가져와",
  ...
  const riskCount =
    countHits(lower, copyrightRiskTerms) +
    countHits(lower, responsibilityRiskTerms) +
    countHits(lower, agencyRiskTerms);
  return Math.min(100, riskCount * 34);
}

function calculateEthicsRisk({ biasRisk, hallucinationRisk, groundingScore, safetyRisk, conductRisk }) {
  ...
  return Math.round(
    Math.min(
      ...
      Math.max(
        safetyRisk * 0.7,
        conductRisk * 0.72,
        biasRisk * 0.2 + hallucinationRisk * 0.3 + groundingRisk * 0.18 + safetyRisk * 0.22 + conductRisk * 0.24
      )
    )
  );
}

function buildEthicsRiskComment({ ethicsRisk, biasRisk, hallucinationRisk, groundingScore, safetyRisk, conductRisk }) {
  if (safetyRisk >= 50) {
    return "주의가 필요합니다. 값을 가려도 개인정보를 알려주거나 더 묻는 태도 자체가 안전 기준과 충돌할 수 있습니다.";
  }
  if (conductRisk >= 50) {
    return "주의가 필요합니다. 저작권, 책임, 사용자의 판단권을 가볍게 넘기는 표현이 윤리 기준과 충돌할 수 있습니다.";
  }
  if (ethicsRisk >= 70) {
